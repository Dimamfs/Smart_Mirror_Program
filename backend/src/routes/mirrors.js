...
// ── GET /api/mirrors/active-user/:mirrorId ────────────────────────────────────
// Polled every 5 s by the mirror UI to know who is the active user.
// Falls back to the first profile linked via mirror_id if active_mirror_users
// has no row yet (backwards-compatible with the old pairing flow).
router.get('/active-user/:mirrorId', async (req, res, next) => {
  try {
...
// ── GET /api/mirrors/:mirrorId/gmail/status ───────────────────────────────────
// Returns whether Gmail is connected for the active user on this mirror.
router.get('/:mirrorId/gmail/status', async (req, res, next) => {
  try {
    const profile = await getActiveProfile(req.params.mirrorId);
    if (!profile) {
...
// ── GET /api/mirrors/:mirrorId/gmail/messages ─────────────────────────────────
// Returns unread inbox summary for the active user on this mirror.
// Does NOT expose tokens — calls gmailService which handles auth internally.
router.get('/:mirrorId/gmail/messages', async (req, res, next) => {
  try {
    const profile = await getActiveProfile(req.params.mirrorId);
    ...
    // If the token is expired / revoked, return empty rather than 500
    if (err.status === 404) {
      return res.json({ messages: [] });
    }
    next(err);
...
// ── GET /api/mirrors/:mirrorId/spotify/status ─────────────────────────────────
router.get('/:mirrorId/spotify/status', async (req, res, next) => {
  try {
    const profile = await getActiveProfile(req.params.mirrorId);
    if (!profile) return res.json({ connected: false, displayName: null });
    res.json({
      connected:   !!profile.spotify_connected,
      displayName: profile.spotify_display_name || null,
    });
  } catch (err) {
    next(err);
  }
...
// ── GET /api/mirrors/:mirrorId/spotify/now-playing ────────────────────────────
router.get('/:mirrorId/spotify/now-playing', async (req, res, next) => {
  try {
    const profile = await getActiveProfile(req.params.mirrorId);
...
// ── GET /api/mirrors/:mirrorId/spotify/player ─────────────────────────────────
// Always returns 200. Returns raw Spotify player object so the mirror widget's
// own normalizer can handle field names (is_playing, item, progress_ms, etc.).
router.get('/:mirrorId/spotify/player', async (req, res) => {
  try {
    const profile = await getActiveProfile(req.params.mirrorId);
...
// ── POST /api/mirrors/:mirrorId/spotify/control ───────────────────────────────
// Body: { action: 'play' | 'pause' | 'next' | 'previous' }
router.post('/:mirrorId/spotify/control', async (req, res) => {
  try {
    const profile = await getActiveProfile(req.params.mirrorId);
    if (!profile || !profile.spotify_connected) {
