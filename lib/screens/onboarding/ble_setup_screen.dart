...
class _BleSetupScreenState extends State<BleSetupScreen> {
    ...
    try {
      ...
      setState(() {
        ...
        _error = "Couldn't find a Smart Mirror nearby.\n\n"
            "Make sure the mirror is powered on and hasn't been set up before, "
            'then try again.';
      });
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _connect() async {
    ...
    try {
      await _provisioner.connectAndDiscover(_device!);
      if (!mounted) return;
  ...
  Future<void> _submit(String password) async {
    ...
    try {
      ...
      await _provisioner.submitCredentials(ssid: actualSsid, password: password);
      if (!mounted) return;
